...
//
// The age of the file is the only input, which is what makes this work across replicas without any coordination:
// on a shared mount, whichever replica gets there first refreshes the file and the others find it fresh and go back to sleep, and on separate disks each replica refreshes its own copy.
