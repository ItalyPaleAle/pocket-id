...
func (s *UserService) CreateUserInternal(ctx context.Context, input dto.UserCreateDto, isLdapSync bool, tx *gorm.DB) (model.User, error) {
	...
	// Membership lives in the join table, so creating the user does not change
	// the group rows on its own. The SCIM sync skips groups whose UpdatedAt
	// predates the remote resource's last-modified time, so without this the new
	// membership would never be pushed to the SCIM provider.
	if len(userGroups) > 0 {
		if err := s.touchUserGroups(ctx, tx, groupIDs(userGroups)); err != nil {
			return model.User{}, err
		}
	}
